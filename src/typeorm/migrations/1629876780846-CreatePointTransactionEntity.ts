	...
	implements MigrationInterface
{
	...
	public async up(queryRunner: QueryRunner): Promise<void> {
		await queryRunner.query(
			`CREATE TABLE \`tripple\`.\`point_transactions\` (\`id\` char(36) NOT NULL, \`eventType\` enum ('REVIEW', 'DIRECT') NOT NULL, \`resource_id\` char(36) NULL, \`user_id\` char(36) NOT NULL, \`amount\` int NOT NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), INDEX \`user_id_index\` (\`user_id\`), INDEX \`source_id_event_type_index\` (\`resource_id\`, \`eventType\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
		)
	}
